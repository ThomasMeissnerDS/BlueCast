...
class PerfectFitPreprocessor(CustomPreprocessing):
    ...
    def fit_transform(self, df: pd.DataFrame, target: pd.Series) -> Tuple[pd.DataFrame, pd.Series]:
    ...
    def transform(
        self, df: pd.DataFrame, target: Optional[pd.Series] = None, prediction_mode: bool = False
