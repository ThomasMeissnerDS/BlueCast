...
def correlation_to_target(df: pd.DataFrame, target: str, show: bool = True) -> go.Figure:
    """
...
def plot_against_target_for_regression(
    ...
    # Create subplots
    fig = make_subplots(
        rows=num_rows, cols=num_cols_grid,
    ...
    fig.update_layout(
        title_text="Regression Analysis",
        height=400 * num_rows,
        showlegend=False
    )
...
def plot_pca(df: pd.DataFrame, target: str, scale_data: bool = True, show: bool = True) -> go.Figure:
    """
...
def plot_pca_biplot(df: pd.DataFrame, target: str, scale_data: bool = True, show: bool = True) -> go.Figure:
    """
...
def plot_theil_u_heatmap(data: pd.DataFrame, columns: List[Union[str, int, float]], show: bool = True) -> go.Figure:
...
def plot_ecdf(
    ...
    if plot_all_at_once:
        ...
        for col in columns:
            ...
            fig.update_layout(
                title_text=f"ECDF and Histogram for {col}",
                showlegend=False
            )
...
def plot_andrews_curve(
    df: pd.DataFrame, target: str, n_samples: Optional[int] = 200, random_state=500, show: bool = True
) -> go.Figure:
    """
    Plot Andrews curve.
    ...
    numeric_cols = df_sample.select_dtypes(include=[np.number]).drop(columns=[target]).columns
    ...
    for target_val in df_sample[target].unique():
        ...
        for idx in subset.index[:min(50, len(subset))]:  # Limit for performance
            ...
            fig.add_trace(go.Scatter(
                ...
                showlegend=(idx == subset.index[0]),  # Only show legend for first trace of each class
                opacity=0.7
            ))
...
def create_eda_dashboard(df: pd.DataFrame, target_col: str, port: int = 8050, run_server: bool = True):
    """
