...
def test_step_feature_engineer(mock_llm, sample_df, tmpdir):
    ...
    orch.context.feature_code_snippets = [
        "df['new_col'] = df['num1'] * 2"
    ]
    
    plan = {"feature_engineering_hints": ["hint 1"]}
    
    with patch("bluecast.ai.orchestrator.Orchestrator._get_critique_rounds", return_value=0):
...
def test_evaluate_for_arch(mock_llm, sample_df, tmpdir):
    ...
    with patch("bluecast.ai.orchestrator.Orchestrator._get_critique_rounds", return_value=0):
        ...
        sugg = orch._evaluate_for_arch("xgboost", "XGBoost", result, iteration=0, total_iterations=2)
