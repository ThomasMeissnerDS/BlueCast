...
def test_check_load_confs_defaults(default_model):
    with patch('bluecast.ml_modelling.xgboost_regression.logger') as mock_logger:
        ...
        assert isinstance(default_model.conf_params_xgboost, XgboostRegressionFinalParamConfig)
...
def test_check_load_confs_partial():
    ...
    model = XgboostModelRegression(class_problem="regression", conf_training=conf_training)
