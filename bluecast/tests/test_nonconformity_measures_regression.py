...
def test_absolute_error_loss():
    ...
    assert absolute_error(y_true_regression, synthetic_results_regression) == np.asarray(
        [10.,  0.,  5.]
    )
...
def test_normalized_error_error_loss():
    synthetic_results_regression, y_true_regression = create_synthetic_regression_data()
    assert normalized_error(y_true_regression, synthetic_results_regression, 5) == np.asarray(
        [2., 0., 1.]
    )
