...
def remove_correlated_columns(df: pd.DataFrame, threshold: float = 0.9):
    ...
    :param threshold: Float indicating the correlation threshold. If the absolute correlation is
        above or equal this value, one of the columns will be dropped.
    :return: New DataFrame with reduced number of columns.
    """
    df = df.copy()
    col_corr = set()
    corr_matrix = df.corr()
    ...
    df = df.drop(columns=list(col_corr), errors="ignore")
