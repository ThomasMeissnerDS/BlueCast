...
checks are executed on the test ata and whenever a `predict` is
executed. Like with other components users can replace the default
...
class DataDrift(BaseClassDataDrift):
    ...
    def fit_data_drift(self, data: pd.DataFrame, anonymize_categories: bool = True, **params) -> Dict[str, Any]:
        """
    ...
    def check_drift(
        self, new_data: pd.DataFrame, threshold: float = 0.05, anonymize_categories: bool = True, **params
    ) -> Dict[str, bool]:
        """
        ...
        for column in new_data.columns:
            ...
            if pd.api.types.is_numeric_dtype(new_data[column]):
                ...
                if anonymize_categories:
                    value_counts_new = value_counts_new.sort_index().reset_index(drop=True)
...
automl = BlueCast(
        class_problem="binary",
        target_column="target",
        custom_data_drift_detector=data_drift_checker,
    )

automl.fit_eval(df_train, df_eval, y_eval, target_col="target")

# print the results on demand
print(automl.custom_data_drift_detector)
```
