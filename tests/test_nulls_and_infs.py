...
def test_fill_nulls():
    ...
    expected_output = pd.DataFrame({'A': [1, 0, 3, 4],
                                    'B': [0, 6, 7, 0]})
    assert fill_nulls(df, fill_with=0).astype(float).equals(expected_output.astype(float))

    # Test case 2: Fill null values with NaN
    df_alt = pd.DataFrame({'A': [1, float('nan'), 3, 4],
                                    'B': [float('nan'), 6, 7, float('nan')]})
